...
# Solo va a haber un ms en vuelo para simplificar el protocolo
MAX_MSG_IN_FLIGHT = 1 
...
# El msg_id es un número de 8 bits, por lo que el rango es de 0 a 255
# Un msg partido en varios mensajes tendrá el mismo msg_id, y el último mensaje tendrá end=1
class SocketProtocol:
    ...
    def wait_for_ack(self) -> None:
        while self.in_flight.__len__() >= self.max_msg_in_flight:
            try:
                packet = self._recv()
                if packet.is_ack:
                    # Acá deberia eliminar todos los mensajes en vuelo con msg_id menor o igual al recibido, 
                    # pero como solo hay un mensaje en vuelo, elimino el que tenga el mismo msg_id
                    if packet.msg_id in self.in_flight:
                        del self.in_flight[packet.msg_id]
                    else:
                        raise ValueError(f"Received ACK for unknown message ID: {packet.msg_id}")
                else:
                    raise ValueError(f"Received non-ACK message when expecting ACK: {packet.msg_id}")
                ...
                # Reenviar todos los mensajes en vuelo
                # No hago una copia ni itero la lista xq solo hay un ms en vuelo
                msg_id, data = next(iter(self.in_flight.items()))
                self.send_message(data, msg_id, False)



    def send_message(self, data: bytes, msg_id: int | None, wait_for_ack: bool = True) -> None:
        ...
        while len(view) - bytes_sent > MAX_MESSAGE_SIZE:
            self._send(view[bytes_sent : bytes_sent + MAX_MESSAGE_SIZE], 0, msg_id, False)
    ...
    def _recv(self) -> SocketPacket:
        ...
        while end == 0:
            ...
            if next_packet.msg_id != msg_id:
                raise ValueError(f"Message ID mismatch: expected {msg_id}, got {next_packet.msg_id}")
